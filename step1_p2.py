...
# new_img.draw(window) # Check below to see why this is commented out.
...
for x in range(1280):
	for y in range(720):
		new_img.setPixel(x, y, color_rgb(0,0,255))
new_img.draw(window) # This needs to be after the pixels are set.


"""
I have commented out the previous usage of the .draw() function because it is useless in
this case and had caused my code to take way longer to run. The instructions don't state
this, but with the .draw() being called after the double for loops, the blue
rectangle is displayed, as when I didn't have it after the double for loop,
nothing would be displayed and my program would stop responding. 

Moreover, if I run `new_img.draw(window)` two times it gives an error.
"""
